...
def load_data(data_dict, aberation=None):
    ...
    output = load_mag_data(
        start, end, frame, sampl, folder_path, kernel_path, re_sampling=freq_sampl,
    )
    pre_df = {
        'time': output['time'],
        'Bx': output['b'][0],
        'By': output['b'][1],
        'Bz': output['b'][2],
        'X': output['position'][0] / mars_radius,
        'Y': output['position'][1] / mars_radius,
        'Z': output['position'][2] / mars_radius,
    }
    ...
    df_orbits = pd.read_csv(
        orbits_file_path, sep=', ', header=2, engine='python',
    )  # , names=["a", "b", "c"])
    df_orbits = df_orbits[
        (df_orbits['START_ET'] >= t_start - 86400 * 2)
        & (t_end + 86400 >= df_orbits['START_ET'])
    ]
    ...
    if aberation is not None:
        ...
        df['X_aber'], df['Y_aber'], df['Z_aber'] = np.array(
            np.dot(
                Rot(np.deg2rad(aberation), axis='z'),
                np.array([df['X'], df['Y'], df['Z']]),
            ),
        )
        df['Bx_aber'], df['By_aber'], df['Bz_aber'] = np.array(
            np.dot(
                Rot(np.deg2rad(aberation), axis='z'),
                np.array([df['Bx'], df['By'], df['Bz']]),
            ),
        )
...
def plot_crossings(
    df, bow_shock_pars={'x0': 0.74, 'L': 1.82, 'eps': 1.01}, markerstyle='-',
):
    ...
    yz_proj_sw = np.sqrt(
        df.loc[df['loc'] == 1, 'Y' + subsc].values ** 2
        + df.loc[df['loc'] == 1, 'Z' + subsc].values ** 2,
    )
    yz_proj_ms = np.sqrt(
        df.loc[df['loc'] == 0, 'Y' + subsc].values ** 2
        + df.loc[df['loc'] == 0, 'Z' + subsc].values ** 2,
    )
    ax.plot(
        df.loc[df['loc'] == 1, 'X' + subsc].values,
        yz_proj_sw,
        f'b{markerstyle}',
        markersize=1,
        label='Solar wind',
    ...
    ax.plot(
        df.loc[df['loc'] == 0, 'X' + subsc].values,
        yz_proj_ms,
        f'r{markerstyle}',
        markersize=1,
        label='Induced magnetosphere',
    )
    ax.plot(x0, 0, 'r*', markersize=13)

    circle_yz_proj = patches.Circle(
        (0, 0), 1, edgecolor='black', facecolor='none', linewidth=2,
    )
    # ax.axhline(y=0, color ='k', ls = '--')
    ax.set_ylim([-0.1, 3])
    ax.set_xlim([-3, 3])
    ax.set_ylabel(
        '$\\sqrt{Z_{\\rm MSO} ^2 + Y_{\\rm MSO}^2}$ $[R_{\\rm M}]$', fontsize=14,
    )
    ...
    ax.plot(
        ...
        color='black',
        linestyle='--',
        label='Bow shock conic section: ' + r'$L(1+\epsilon \cos(\theta))^{-1}$',
    )
...
def clock_angles(
    ...
    for i, j in zip(range(len(idx_sw2ms)), range(len(idx_ms2sw))):
        ...
        orbit_id = (
            min(df_orbits.loc[(df_orbits['PERIAPSIS_ET'] > df.loc[J, 'time'])].index)
            - 1
        )
        ...
        if (df.loc[J, 'time'] >= orbit_start) and (df.loc[I, 'time'] <= orbit_end):
            ...
            if J != idx_ms2sw[-1]:
                ...
                K = idx_ms2sw[j + 1]
                if abs(df.loc[K, 'time'] - df.loc[I, 'time']) <= (
...
def rotation_to_mse(df, remove):
    ...
    B_mso = np.array(
        [df['Bx' + subsc].values, df['By' + subsc].values, df['Bz' + subsc].values],
    )
    B_mse = np.zeros(shape=B_mso.shape) * np.nan

    pos_mso = np.array(
        [df['X' + subsc].values, df['Y' + subsc].values, df['Z' + subsc].values],
    )
...
def rotated_to_mse(
    ...
    t_start = df.loc[df.index[0], 'time']
    t_end = df.loc[df.index[-1], 'time']
    freq_sampl = data_dict['freq_sampl']
    ...
    if save[0]:
        ...
        file_all = (
            'mag_mse_'
            + spice.timout((t_start), pictur='YYYY-MM-DD')
            + '_'
            + spice.timout((t_end), pictur='YYYY-MM-DD')
            + '_n'
            + str(freq_sampl)
            + save[1]
            + '.csv'
...
def plot_mse(
    df, plot_dict=dict(y_lim=[-1000, 1000]), plot_crossings=dict(plot='False'),
):
    ...
    fig, ax = plt.subplots(
        nrows=plot_n, ncols=1, figsize=(fig_width, fig_height), sharex=True,
    )
    ...
    if 'x_lim' in plot_dict.keys():
        ...
        x_lim = [
            spice.utc2et(plot_dict['x_lim'][0]),
            spice.utc2et(plot_dict['x_lim'][1]),
        ]
    ...
    times_ticks_txt = []
    times_ticks_txt.append(
        spice.timout(times_ticks[0], pictur='HR:MN')
        + '\n'
        + spice.timout(times_ticks[0], pictur='YYYY-MM-DD'),
    )
    times_ticks_txt += [
        spice.timout(times_ticks[i], 'YYYY-MM') for i in range(1, len(times_ticks))
    ]
    ...
    ax[4].plot(
        t_plot,
        np.sqrt(df['Bx'] ** 2 + df['By'] ** 2 + df['Bz'] ** 2),
        '.k',
        markersize=markersize,
        label='Bz',
    )
    ax[4].set_ylabel(r'$|B| \, [nT]$', fontsize=14)
...
def plot_space_coverage(
    df, bow_shock_pars={'x0': 0.74, 'L': 1.82, 'eps': 1.01}, markersize=1,
):
    ...
    ax[0].plot(
        df['X_mse'], df['Z_mse'], 'r.', markersize=markersize, label='trajectory in MSE',
    )
    ...
    ax[1].plot(
        df['Y_mse'], df['Z_mse'], 'r.', markersize=markersize, label='trajectory in MSE',
    )
