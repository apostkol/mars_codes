...
def mollweide_maps(
    grid,
    maps=['Br_sm', 'Bth_sm', 'Bphi_sm', 'measurements_counts'],
    ...
    if save:
        ...
        plots_path_date = os.path.abspath(
            f'{plots_path}/{grid.data}_dr_{grid.dr}_dth_{grid.dth_deg}_dphi_{grid.dphi_deg}_{map_type_name}',
        )
    ...
    for ir in range(len(grid.r)):
        ...
        if len(maps) == 4:
            ...
            map4 = ax[3].pcolormesh(
                phi_mol[:, :, ir], th_mol[:, :, ir], u[3], vmin=vmin, vmax=vmax,
            )
        ...
        fig.suptitle(
            rf'$h = {str((np.around((grid.r[ir]-1)*3396.19, 0)))} km$ $ ( r = {str(np.around(grid.r[ir], 3))} R_{{M}})$',
            fontsize=fts,
        )
        ...
        if save:
            fig.savefig(
                f'{plots_path_date}/{map_type_name}_r_{dist}_Rm.png',
                bbox_inches='tight',
            )
