...
def rot_axis_angle(v1, v2):
    ...
    z_unit = [0, 0, 1]
    c = cos_angle = np.dot(N_unit, z_unit) / (
        np.linalg.norm(N_unit) * np.linalg.norm(z_unit)
    )
    angle = np.arccos(c)
...
def rotate_to_xy(v1, v2):
    ...
    v1_xy = np.dot(matrix, v1)
    v2_xy = np.dot(matrix, v2)
...
def plotly_trisurf(x, y, z, simplices, face_color='blue', opacity=0.9, plot_edges=None):
    ...
    triangles = go.Mesh3d(
        x=x, y=y, z=z, i=I, j=J, k=K, name='', color=face_color, opacity=opacity,
    )
