...
class Grid:
    ...
    def __init__(
        self, grid_dict, load_from_file=False, file_path=None, data=None, grid_name=None,
    ):

        for attr in grid_dict.keys() - {
            'cells',
            'r_bins',
            'th_bins',
            'phi_bins',
        ...
        self.grid_name = (
            grid_name if 'grid_name' not in grid_dict.keys() else grid_dict['grid_name']
        )
        ...
        if self.load_from_file == False:
            ...
            (
                ...
                self.th,
                self.phi,
            ) = self._set_grid_cells(
                self.r_lim, th_lim, phi_lim, self.dr, self.dth, self.dphi,
            )
                    ...
                    for direction in [-1, +1]:
                        ...
                        for c in cells:
                            ...
                            if (
                                ...
                                == c.r_bin
                                and cell.neighbours_bins[coordinate][direction][
                                    'th_bin'
                                ]
                                == c.th_bin
                                and cell.neighbours_bins[coordinate][direction][
    ...
    def current_densities(self, radius):
        ...
        if self.load_from_file:
            ...
            for cell in self.cells:
                ...
                cell.divB_curlB = abs(
                    div(cell.attr_dict(), cell.neighbours, field='B'),
                ) / np.linalg.norm(curlB)

                cell.Jr, cell.Jth, cell.Jphi = J[0], J[1], J[2]

            for cell in self.cells:
                cell.divJ_magJ = abs(
                    div(cell.attr_dict(), cell.neighbours, field='J'),
                ) / np.sqrt(cell.Jr**2 + cell.Jth**2 + cell.Jphi**2)
                ...
                cell.sigma_Jr = (
                    ...
                    * np.sqrt(
                        (
                            ...
                            / (cell.r_c * radius * 1e3)
                            * sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'phi', 'th',
                            )
                        )
                        ** 2
                        + (
                            np.cos(cell.th_c)
                            / (cell.r_c * radius * 1e3 * np.sin(cell.th_c))
                            * cell.sigma_Bphi
                        )
                        ** 2
                        + (
                            ...
                            / (cell.r_c * radius * 1e3 * np.sin(cell.th_c))
                            * sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'th', 'phi',
                            )
                        )
                        ** 2,
                    )
                ...
                cell.sigma_Jth = (
                    ...
                    * np.sqrt(
                        (
                            ...
                            / (cell.r_c * radius * 1e3 * np.sin(cell.th_c))
                            * sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'r', 'phi',
                            )
                        )
                        ** 2
                        + (
                            sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'phi', 'r',
                            )
                            / (radius * 1e3)
                        )
                        ** 2
                        + (1 / (cell.r_c * radius * 1e3) * cell.sigma_Bphi) ** 2,
                    )
                )  # / abs(cell.Jth)
                ...
                cell.sigma_Jphi = (
                    ...
                    * np.sqrt(
                        (
                            sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'th', 'r',
                            )
                            / (radius * 1e3)
                        )
                        ** 2
                        + (1 / (cell.r_c * radius * 1e3) * cell.sigma_Bth) ** 2
                        + (
                            1
                            / (cell.r_c * radius * 1e3)
                            * sigma_gradient(
                                cell.attr_dict(), cell.neighbours, 'B', 'r', 'th',
                            )
                        )
                        ** 2,
                    )
            ...
            for cell in self.cells:
                ...
                for coordinate in cell.neighbours.keys():
                    ...
                    for direction in cell.neighbours[coordinate].keys():
                        ...
                        if cell.neighbours[coordinate][direction] == None:
                            ...
                            neighbour_dict = {
                                k: cell.neighbours[coordinate][direction].__dict__[k]
                                for k in cell.neighbours[coordinate][
                                    direction
                                ].__dict__.keys()
                                - {
                                    'r',
                ...
                cell.divB_curlB = abs(
                    div(cell.attr_dict(), neighbours_dict, field='B'),
                ) / np.linalg.norm(curlB)
            ...
            for cell in self.cells:
                ...
                for coordinate in cell.neighbours.keys():
                    ...
                    for direction in cell.neighbours[coordinate].keys():
                        ...
                        neighbours_dict[coordinate].update({direction: neighbour_dict})
                cell.divJ_magJ = abs(
                    div(cell.attr_dict(), neighbours_dict, field='J'),
                ) / np.sqrt(
                    cell.Jr**2 + cell.Jth**2 + cell.Jphi**2,
                )  # /(radius*1e3)
                ...
                cell.sigma_Jr = (
                    ...
                    * np.sqrt(
                        (
                            ...
                            / (cell.r_c * radius * 1e3)
                            * sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'phi', 'th',
                            )
                        )
                        ** 2
                        + (
                        ...
                        ** 2
                        + (
                            ...
                            / (cell.r_c * radius * 1e3 * np.sin(cell.th_c))
                            * sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'th', 'phi',
                            )
                        )
                        ** 2,
                    )
                ...
                cell.sigma_Jth = (
                    ...
                    * np.sqrt(
                        (
                            ...
                            / (cell.r_c * radius * 1e3 * np.sin(cell.th_c))
                            * sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'r', 'phi',
                            )
                        )
                        ** 2
                        + (
                            sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'phi', 'r',
                            )
                            / (radius * 1e3)
                        )
                        ** 2
                ...
                cell.sigma_Jphi = (
                    ...
                    * np.sqrt(
                        (
                            sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'th', 'r',
                            )
                            / (radius * 1e3)
                        )
                        ** 2
                        ...
                        + (
                            ...
                            / (cell.r_c * radius * 1e3)
                            * sigma_gradient(
                                cell.attr_dict(), neighbours_dict, 'B', 'r', 'th',
                            )
                        )
                        ** 2,
                    )
    ...
    def grid_to_dict(self):
        ...
        if not self.load_from_file:
            save_dict = {
                k: grid_dict[k] for k in grid_dict.keys() - {'cells', 'save_dict'}
            }
        else:
            ...
            for coordinate in cell_dict['neighbours'].keys():
                ...
                for direction in cell_dict['neighbours'][coordinate].keys():
                    ...
                    if cell_dict['neighbours'][coordinate][direction] is not None:
                        ...
                        if isinstance(
                            cell_dict['neighbours'][coordinate][direction], dict,
                        ):
                            neighbour_dict = cell_dict['neighbours'][coordinate][
                                direction
                            ]
    ...
    def currents_uncertainties(self, radius):
        ...
        for cell in self.cells:
            ...
            cell.Ith = (
                ...
                * cell.dphi
                * (radius * 1e3) ** 2
                if not np.isnan(cell.Jth)
                else np.nan
            )
            cell.Iphi = (
                cell.Jphi * cell.r_c * cell.dr * cell.dth * (radius * 1e3) ** 2
                if not np.isnan(cell.Jphi)
                else np.nan
            )
        ...
        for cell in self.cells:
            ...
            if cell.neighbours['r'][+1] is not None:
                if not np.isnan(cell.neighbours['r'][+1].Jr):
                    dIr += (
                        cell.neighbours['r'][+1].Jr
                        * (cell.neighbours['r'][+1].r_c) ** 2
                        * np.sin(cell.th_c)
                        * cell.dth
                        * cell.dphi
                        * (radius * 1e3) ** 2
            ...
            if cell.neighbours['th'][+1] is not None:
                if not np.isnan(cell.neighbours['th'][+1].Jth):
                    dIth += (
                        cell.neighbours['th'][+1].Jth
                        * cell.r_c
                        * np.sin(cell.neighbours['th'][+1].th_c)
                        * cell.dr
                        * cell.dphi
                        * (radius * 1e3) ** 2
                    )
            ...
            cell.dI_I = abs(dIr + dIth + dIphi) / np.sqrt(
                cell.Ir**2 + cell.Ith**2 + cell.Iphi**2,
            )
    ...
    def save_grid(self):
        ...
        with open(
            f"grid_{self.grid_name}__{re.sub('.csv','', re.sub(r'.*/' ,'', self.data))}.pickle",
            'wb',
        ) as f:
            self.grid_to_dict()
